        ...
        .then(user => {
          ...
          user.save(function(e, user) {
            ...
            if (global.config.emailSendEnable) {
              sendMail(
                user.email,
                `${res.__("Регистрация на сайте")} ${global.config.host}`,
                `${res.__("Верифицируйте свой")} <a href="${global.config.origin}/email/${user.token}" target="_blank">${res.__("емейл адрес")}</a>`,
                (e, d) => {
                  if (e) {
                    console.log(e);
...
module.exports.access = (req, res, next) => {
  ...
  if (!("email" in req.body) || !("g-recaptcha-response" in req.body) || !req.body["g-recaptcha-response"])
        ...
        (e, r, b) => {
          ...
          user
            ...
            .then(user => {
              sendMail(
                user.email,
                res.__("Восстановление доступа на сайте") + " " + config.host,
                res.__("Для восстановления доступа пройдите по ссылке") + ` <a href="${config.origin}/access/${user.token}">${res.__("восстановить доступ")}.</a>`,
                (e, d) => {
                  if (e) {
                    console.log(e);
                    req.flash("error", res.__("Не удалось отправить письмо для верификации на Вашу почту."));
    ...
    db.User.findOne({ email: req.user.email }, (e, user) => {
      ...
      user.save((e, user) => {
        ...
        if (global.config.emailSendEnable) {
          sendMail(
            user.email,
            `${res.__("Верификация емейл адреса на сайте")} ${global.config.host}`,
            `${res.__("Верифицируйте свой")} <a href="${global.config.origin}/email/${user.token}" target="_blank">${res.__("емейл адрес")}</a>`,
            (e, d) => {
              if (e) {
                console.log(e);
